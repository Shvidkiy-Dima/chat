...
class TestUser(LoginTestCase):
    ...
    def test_change_image(self):
        ...
        user_data = ChatUserSerializer(self.current_user).data
        self.assertTrue(user_data['image'] == settings.DEFAULT_IMAGE)
        default_image_path = path.join(settings.STATIC_ROOT,
                                       settings.DEFAULT_IMAGE.lstrip(settings.STATIC_URL))

        with TemporaryDirectory() as temp_dir, open(default_image_path, 'rb') as test_img:

            # Now all images will be saved in temp dir
            settings.MEDIA_ROOT = temp_dir
            settings.DEL_OLD_IMAGES = True
            ...
            #Check that we change user image again and delete previous img
